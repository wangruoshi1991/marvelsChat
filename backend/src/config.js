...
const parseMediaRetrievalBaseUrl = (value) => {
  ...
  try {
    ...
    throw new Error(
      "MEDIA_RETRIEVAL_DASHSCOPE_API_BASE_URL must be an HTTPS regional or workspace /api/v1 URL.",
    );
  }
  return normalized;
};
...
export const createMediaRetrievalConfig = (env = process.env) => ({
  ...
  dashscopeApiBaseUrl: parseMediaRetrievalBaseUrl(
    env.MEDIA_RETRIEVAL_DASHSCOPE_API_BASE_URL,
  ),
  ...
  captionModelVersion: parseRequiredString({
    ...
    fallback: String(
      env.MEDIA_RETRIEVAL_CAPTION_MODEL || "qwen3.6-flash",
    ).trim(),
  }),
  ...
  embeddingModelVersion: parseRequiredString({
    ...
    fallback: String(
      env.MEDIA_RETRIEVAL_EMBEDDING_MODEL || "qwen3-vl-embedding",
    ).trim(),
  }),
...
export const getMediaRetrievalConfigStatus = (runtimeConfig) => {
  ...
  return {
    ...
    captionModelVersion:
      ...
      || retrieval?.captionModel
      || "qwen3.6-flash",
    embeddingModel: retrieval?.embeddingModel || "qwen3-vl-embedding",
    embeddingModelVersion:
      retrieval?.embeddingModelVersion
      || retrieval?.embeddingModel
      || "qwen3-vl-embedding",
    embeddingDimension:
      retrieval?.embeddingDimension
      || MEDIA_RETRIEVAL_LIMITS.embeddingDimension,
    embeddingNormalization:
      retrieval?.embeddingNormalization || "provider-native-dense-v1",
    maxVideoFrames:
      retrieval?.maxVideoFrames || MEDIA_RETRIEVAL_LIMITS.maxVideoFrames,
