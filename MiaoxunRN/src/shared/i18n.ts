...
export const appErrorText = (
  ...
  if (value.includes('超过 1 分钟') || value.includes('over 1 minute')) {
    return textFor(
      language,
      '消息发出超过 1 分钟，不能撤回',
      'Messages can only be recalled within 1 minute',
    );
  }
  if (value.includes('before recall tracking')) {
    return textFor(
      language,
      '这条历史消息不支持撤回',
      'This earlier message cannot be recalled',
    );
  }
...
export const displayLocationText = (
  language: Language,
...
export const displayText = (language: Language, rawValue?: string | null) => {
  ...
  const mappedValues: Record<string, string> = {
    ...
    '把聊天、内容、小站和 Agent 连在一起。':
      'Connects chats, content, stations, and agents.',
...
export const authErrorText = (language: Language, message?: string | null) => {
  ...
  if (value.includes('account not found')) {
    return textFor(
      language,
      '账号不存在，请使用昵称、手机号或邮箱登录',
      'Account not found. Use name, phone, or email',
    );
  }
  ...
  if (
    ...
    value.includes('password must be')
  ) {
    return textFor(
      language,
