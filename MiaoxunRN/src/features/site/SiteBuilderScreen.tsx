...
export function SiteBuilderScreen({
  ...
  return (
    ...
    >
      ...
      <View
        style={[
          styles.siteBuilderHero,
          { backgroundColor: palette.surface, borderColor: palette.border },
        ]}
      >
        <View
          style={[
            styles.siteBuilderHeroIcon,
        ...
        <Text
          style={[styles.siteBuilderBody, { color: palette.secondaryText }]}
        >
          {textFor(
            language,
            '这里会保留给后续 AI 建站与工具执行能力。当前先独立存在。',
            'This entry is reserved for future AI site building and tool actions. It now stays separate from the Butler chat.',
          )}
        </Text>
      </View>
      <View style={styles.siteBuilderCapabilities}>
        <SiteBuilderCapabilityRow
          ...
          subtitle={textFor(
            language,
            '后续从这里接入页面生成、模块装配和站点发布。',
            'This will later host page generation, module assembly, and site publishing.',
          )}
        />
        <SiteBuilderCapabilityRow
          icon={Layers}
          ...
          subtitle={textFor(
            language,
            '后续接入站点素材、文件和多步骤任务执行。',
            'This will later connect assets, files, and multi-step task execution.',
          )}
        />
      </View>
    </ScrollView>
...
function SiteBuilderCapabilityRow({
  ...
  return (
    <View
      style={[
        styles.siteBuilderCapabilityRow,
        { backgroundColor: palette.surface, borderColor: palette.border },
      ]}
    >
      <View
        style={[
          styles.siteBuilderCapabilityIcon,
          { backgroundColor: `${palette.mint}24` },
        ]}
      >
      ...
      <View style={styles.siteBuilderCapabilityCopy}>
        <Text
          style={[styles.siteBuilderCapabilityTitle, { color: palette.text }]}
        >
          {title}
        </Text>
        <Text
          style={[
            styles.siteBuilderCapabilitySubtitle,
