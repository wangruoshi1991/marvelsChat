...
export function QRCodeSheet({
  ...
  const refresh = () => {
    ...
    setCodePayload(
      `miaoxun://ai/${aiId}?e=${Math.floor(
        nextExpiry / 1000,
      )}&s=${nextStyleStep}&n=${Math.random().toString(36).slice(2, 10)}`,
    );
  };

  const remaining = Math.max(0, Math.floor((expiresAt - now) / 1000));
  const remainingText = `${String(Math.floor(remaining / 60)).padStart(
    2,
    '0',
  )}:${String(remaining % 60).padStart(2, '0')}`;
  const qrDiyStyle = useMemo(
    () => qrDiyStyleFor(codePayload || aiId),
    [aiId, codePayload],
  );
    ...
    <>
      ...
      >
        <View style={styles.qrHeader}>
          ...
          <Text
            style={[styles.qrTitle, { color: palette.text }]}
            numberOfLines={1}
          >
          ...
          <View
            style={[
              styles.qrTimer,
              {
                backgroundColor: `${palette.mint}1f`,
                borderColor: `${palette.mint}38`,
              },
            ]}
          >
            <Text style={[styles.qrTimerText, { color: palette.mint }]}>
              {remainingText}
            </Text>
          </View>
          <Pressable
        ...
        <ScrollView contentContainerStyle={styles.qrContent}>
          <View
            style={[
              styles.qrDiyFrame,
              {
                ...
                transform: [{ rotate: qrDiyStyle.rotate }],
              },
            ]}
          >
            <View
            ...
            >
              ...
              <Text
                style={[
                  styles.qrHandMark,
                  { color: qrDiyStyle.palette.accent },
                ]}
              >
                MIAO
              </Text>
          ...
          >
            ...
            <Text style={[styles.qrAiIdText, { color: palette.text }]}>
              AI ID {aiId}
            </Text>
          </View>
        </ScrollView>
