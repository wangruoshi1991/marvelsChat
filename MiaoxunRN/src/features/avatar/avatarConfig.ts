...
export const avatarAccentColors: Record<AvatarAccent, {primary: string; secondary: string; detail: string}> = {
...
export const avatarSkinColors: Record<AvatarSkinTone, {base: string; shade: string; blush: string}> = {
...
export const avatarClothingColors: Record<AvatarTop, {primary: string; secondary: string; line: string}> = {
...
export const avatarBottomColors: Record<AvatarBottom, {primary: string; secondary: string; line: string}> = {
...
export const avatarShoeColors: Record<AvatarShoes, {primary: string; sole: string; line: string}> = {
...
const hexToFloat4 = (hex: string): [number, number, number, number] => {
  ...
  const parse = (start: number) => parseInt(normalized.slice(start, start + 2), 16) / 255;
...
export const avatarMaterialFor = (color: string, roughnessFactor = 0.5) => ({
  ...
  parameters: {
    ...
    emissiveFactor: hexToFloat4(color).map((value, index) => (index === 3 ? 1 : Math.min(1, value * 0.32))) as [number, number, number, number],
    roughnessFactor,
    metallicFactor: 0,
    reflectance: 0.62,
  },
...
export const normalizeAvatarConfig = (config?: AvatarConfigDTO | null): Required<AvatarConfigDTO> => ({
  ...defaultAvatarConfig,
  ...(config || {}),
  version: 2,
  accent: config?.accent || (config?.palette === 'grape' ? 'violet' : config?.palette === 'mono' ? 'mint' : config?.palette) || defaultAvatarConfig.accent,
  accessory: config?.accessory === 'cap' || config?.accessory === 'spark' || config?.accessory === 'glasses' || config?.accessory === 'headphones'
    ? config.accessory
    : defaultAvatarConfig.accessory,
  mouthStyle: config?.mouthStyle || (config?.expression === 'focus' ? 'confident' : config?.expression === 'calm' ? 'calm' : defaultAvatarConfig.mouthStyle),
  top: config?.top || (config?.outfit === 'campus' ? 'uniform' : config?.outfit === 'tech' ? 'jacket' : config?.outfit === 'artist' ? 'shirt' : config?.outfit === 'sport' ? 'sweater' : defaultAvatarConfig.top),
  bottom: config?.bottom || (config?.outfit === 'sport' ? 'track' : config?.outfit === 'artist' ? 'shorts' : defaultAvatarConfig.bottom),
  action: config?.action || (config?.pose === 'hello' ? 'wave' : config?.pose === 'ready' ? 'cross-arms' : defaultAvatarConfig.action),
});
