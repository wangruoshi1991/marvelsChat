...
- direct 好友聊天使用 `/api/realtime` WebSocket 实时通道接收在线消息；登录期间连接按 token 生命周期保持，不因进入或退出某个聊天页、收到 `presence.changed` 或更新本人 `presenceMode` 而重建。WebSocket 异常断开后只重连同一个实时通道，不用短轮询伪装实时；全局 30 秒同步只负责列表、通知和离线后的数据补齐。
...
- 小站页已接入位置入口。点击小站里的“我的社区”或“我的活动区域”进入位置选择；小站形象当前使用 `avatar_config` 驱动的 SVG 卡通展示，保留横向滑动旋转，不再接入不稳定的 3D 渲染验证链路，也不提供未达上线标准的形象编辑入口。舞台透明手势层只在横向滑动意图明确后接管触摸，旋转链路不写入缩放、平移或重置位置。设置页只保留账号、显示、隐私和通知等全局设置。
...
- 小站形象由 `avatar_config` v2 驱动，注册时后端按 AI ID/账号信息生成初始配置，用于小站舞台和用户小头像的一致展示。此前的旧 3D GLB 验证链路已从主流程移除；正式恢复 3D 人物前，必须先完成授权角色模型、贴图、骨骼动画、素材审核、模型压缩、跨平台性能验证和资源版本记录，再作为独立人物系统接入。
- 原型里小站形象、OOTD 和漫画日记封面都是可交互区域；当前 RN 保留形象舞台的旋转展示，OOTD 和日记在对应服务尚未接入时只提示待接入，不写客户端假内容。
